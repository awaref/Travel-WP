...
class Travelo_Importer {
    ...
    function import_content( $file = 'all.xml' ){
        ...
        $importer->fetch_attachments = ( $_POST && key_exists('attachments', $_POST) && $_POST['attachments'] ) ? true : false;
                        ...
                        switch( $_POST['content'] ) {
                            case 'all':
                                if ( $_POST && key_exists('attachments', $_POST) && $_POST['attachments'] ) {
                                    ...
                                    if ( $step === 1 && !file_exists( TRAV_INC_DIR . '/functions/importer/demo_data/media1.xml' ) ) {
                                        $file = 'media.xml';
                                    } else {
                                        $file = 'media' . $step . '.xml';
                                    }
                                    ...
                                    $step++;
                                    $file = 'media'. $step .'.xml';
                                    $file_path     = TRAV_INC_DIR . '/functions/importer/demo_data/'. $file;
                                    ...
                                    $submit_url = add_query_arg( 'step', $step, remove_query_arg( 'step' ) );
                                ?>
                                    <form action="<?php echo esc_url( $submit_url ); ?>" method="post" id="step2submit">
                                    ...
                                    <script>
                                        document.getElementById("step2submit").submit();
                                    </script>
                                <?php
                                    exit;
                                }
            ...
            <form action="<?php echo esc_url( remove_query_arg( 'step' ) ); ?> " method="post">
                ...
                <table class="form-table">
                    ...
                    <tr class="row-attachments hide">
                        ...
                        <td>
                            <fieldset>
                                <label for="attachments"><input type="checkbox" value="1" id="attachments" name="attachments"><?php esc_html_e( 'Import attachments', 'trav' ); ?></label>
                                <p class="description"><?php esc_html_e( 'Download all attachments from the demo may take a while. Please be patient.', 'trav' ); ?></p>
                            </fieldset>
                        </td>
                    </tr>
                </table>
